...
const Allbuyer = () => {
  const { data: buyers = [], refetch } = useQuery({
    queryKey: ["buyers"],
    queryFn: async () => {
      const res = await fetch(`${process.env.REACT_APP_SERVER}/allbuyers`, {
        headers: {
  ...
  const handleDelete = (user) => {
    console.log(user);
    fetch(
      `${process.env.REACT_APP_SERVER}/buyer-orders-delete/${user?.email}`,
      {
        method: "DELETE",
        headers: {
          authorization: `Bearer ${localStorage.getItem("arkDeals")}`,
      ...
      .then((data) => {
        console.log(data);
        fetch(`${process.env.REACT_APP_SERVER}/user-delete/${user?.email}`, {
          method: "DELETE",
          headers: {
            authorization: `Bearer ${localStorage.getItem("arkDeals")}`,
          },
